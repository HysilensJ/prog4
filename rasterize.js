...
var uvBuffers = []; // this contains uv lists by set, in doubles
...
// loads in textures to map to models
// derived from mdn website provided in prog 4 assignment details
function loadTexture( url ) {
    ...
    // Create and load actual texture image
    const image = new Image();
    image.crossOrigin = "anonymous";
    image.onload = () => { 
      gl.bindTexture( gl.TEXTURE_2D, texture );
      gl.texImage2D( gl.TEXTURE_2D, lvl, intFormat, srcFormat, srcType, image );
    ...
    */
    ...
    try {
        if (inputTriangles == String.null)
            ...
            var uvToAdd; // vtx uv to add to the coord array
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                for (whichSetVert=0; whichSetVert<numVerts; whichSetVert++) { // verts in set
                    ...
                    inputTriangles[whichSet].glUVs.push( 1.0 - uvToAdd[0], uvToAdd[1] ); // put uvs in set coord list
...
function setupShaders() {
    ...
    var vShaderCode = `
        ...
        varying vec2 vVertexUV; // vertex uv
